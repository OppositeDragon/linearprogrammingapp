    ...
    for (int i = 0; i < constraintWithSlackAndRightSide.length; i++) {
      ...
      for (int j = 0; j < constraintWithSlackAndRightSide[i].length; j++) {
        if (j == constraintWithSlackAndRightSide[i].length - 1) {
          ...
          rightSideString
              .add(toStringWithSign(constraintWithSlackAndRightSide[i][j], 0, false, true));
        } else {
      ...
      for (int k = j + 1; k < constraints[0].length; k++) {
        ...
        if (isValid) {
          ...
          objectiveFunctionSolution =
              objectiveFunction[0] * oneValue + objectiveFunction[1] * twoValue;
          solutionString =
        ...
        final String header =
            'x_{${whereVarEqualsZero[0]}} \\; y \\;x_{${whereVarEqualsZero[1]}}=0.';
        stepsAlgebraic.add(
          StepsForAlgebraic(
            header: header,
            step: i,
